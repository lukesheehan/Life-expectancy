...
dfnew$Status <- as.character(dfnew$Status)
dfnew$Status[dfnew$Status == "Developing"] <- "1"

dfnew$Status <- as.character(dfnew$Status)
dfnew$Status[dfnew$Status == "Developed"] <- "0"

#dfnew <- dfnew[,-2]
#rownames(dfnew) <- dfnew[,2]

rownames(dfnew) <- make.names(dfnew[,2], unique = TRUE)
...
distance <- get_dist(dfnew)
fviz_dist(distance, gradient = list(low = "#00AFBB", mid = "white", high = "#FC4E07"))
###come back to this with filtered data set

k2 <- kmeans(dfnew, centers = 2, nstart = 25)

str(k2)

fviz_cluster(k2, data = dfnew)
...
k6 <- kmeans(df4, centers = 6, nstart = 25)
